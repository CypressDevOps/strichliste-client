          ...
          return (
            ...
            >
              ...
              <div className='flex gap-3 justify-center'>
                {[1, 2, 3, 4, 5].map((count) => {
                  // Button ist disabled wenn die angeforderte Menge größer ist als der Bestand
                  const isButtonDisabled = stockQuantity === null ? false : stockQuantity < count;

                  return (
                    <button
                      key={count}
                      onClick={() => !isButtonDisabled && onAddProduct(product, count)}
                      ...
                      ${
                        !isButtonDisabled
                          ? 'from-gray-700 to-gray-800 hover:from-green-600 hover:to-green-700 cursor-pointer hover:rotate-3 hover:shadow-2xl hover:shadow-green-500/50'
                          : 'from-gray-600 to-gray-700 cursor-not-allowed opacity-50'
                      }
                      rounded-xl 
                      flex items-center justify-center 
                      transition-all duration-300 
                      ${!isButtonDisabled ? 'hover:scale-125 active:scale-90' : ''}
                      border-2 
                      ${!isButtonDisabled ? 'border-gray-600/50 hover:border-green-400' : 'border-gray-600/30'}
                      overflow-hidden
                      before:absolute before:inset-0 before:rounded-xl
                      before:bg-gradient-to-br before:from-white/0 before:to-white/0
                      ${!isButtonDisabled ? 'hover:before:from-white/20 hover:before:to-transparent' : ''}
                    ...
                    >
                      <img
                        ...
                        alt={`${count}x`}
                        className={`w-12 h-12 relative z-10 ${!isButtonDisabled ? 'group-hover/btn:brightness-125 group-hover/btn:drop-shadow-[0_0_8px_rgba(34,197,94,0.8)]' : 'opacity-60'} transition-all duration-300`}
                      />
